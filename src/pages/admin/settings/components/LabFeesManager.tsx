...
export const LabFeesManager: React.FC<LabFeesManagerProps> = ({
  vendor,
  onUpdate,
}) => {
  ...
  const handleImport = () => {
    try {
      ...
      const importedFees: LabFee[] = rows.map((row) => {
        const [code, description, fee, category, effectiveDate] =
          row.split(",");
        return {
          ...
          >
            <div className="grid grid-cols-5 gap-4 flex-1">
              <input
                ...
                value={fee.code}
                onChange={(e) =>
                  handleUpdateFee(fee.id, { code: e.target.value })
                }
              ...
              <input
                ...
                onChange={(e) =>
                  handleUpdateFee(fee.id, { effectiveDate: e.target.value })
                }
                className="px-2 py-1 border border-gray-200 rounded"
              />
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    handleUpdateFee(fee.id, { received: e.target.checked })
                  }
                  className="rounded border-gray-300"
                  id={`received-${fee.id}`}
                />
                <label
                  htmlFor={`received-${fee.id}`}
